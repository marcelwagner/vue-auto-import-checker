...
import { vueUseComponentsImporter } from "../index.js";
...
describe('tool vuuse-importer', () => {
    ...
    describe('produced', async () => {
        ...
        const vueUseUniqueTags = getUniqueFromList(vueUseResult.tagsList.map((tag) => tag.tagName));
        const vueUseUniqueFiles = getUniqueFromList(vueUseResult.tagsList.map((tag) => tag.file));
