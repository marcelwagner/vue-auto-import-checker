export function writeStats({ stats, filesList, tagsList, uniqueTagsList, showResult, kafka }) {
    ...
    if (showResult) {
        ...
        logger.info(`Unique Tag${uniqueTagsList.length >= 2 ? 's' : ' '}               : ${uniqueTagsList.length}`);
        logger.info(`File${filesList.length >= 2 ? 's' : ' '}                     : ${filesList.length}`);
        logger.info('');
        logger.info('....................................');
        logger.info('');
    }
    ...
    logger.info(`File${stats.fileCounter >= 2 ? 's' : ' '}                     : ${stats.fileCounter}`);
    logger.info(`Template file${stats.templateFiles >= 2 ? 's' : ' '}            : ${stats.templateFiles}`);
