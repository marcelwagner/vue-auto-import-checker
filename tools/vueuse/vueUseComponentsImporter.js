...
import { writeCustomPluginFile } from "../../index.js";
export async function vueUseComponentsImporter(basePath, cachePath) {
    try {
        ...
        for (const fileContentLine of fileContent.split('\n')) {
            const component = fileContentLine.match(/declare const ([\w]+): (vue.DefineComponent|vue1.DefineSetupFnComponent)/);
            if (component === null) {
                continue;
            }
            logger.debug(`found quasar component: ${JSON.stringify(component[1])}`);
