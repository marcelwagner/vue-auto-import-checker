...
- Corrected README.md and commander option for --negate-known
- Corrected README.md commander option for --tool
- Slightly improved README.md
- Slightly improved workflows.md
- Toggled fileParallelism for vitest off, because of failing tests reading cached files, that are emptied by other tests
- Improved error text exposure in failed promises
...
- Updated skills
- package is now 96.2kB in size unpacked (23.4kB in size gzipped)
- cli tests tool refactoring
- Tests run now again parallelly
...
- Replaced prettier with oxfmtrc
- Refactor CLI Options to be more consistent
- Global statistics is now a module object
- Global logger is now a module object
- Workflow run now with node 22 instead of 20 (compatibility for node.js 20 still lasts)

### Added

- Cli Option to output additionally json or markdown (text is default)
- CLI Option to show option config for debugging purposes
- Standalone file getTags for kafka option
- Standalone file getToolTags for tools
- Standalone file getComponentList for components from components.d.ts
- UserConfig module object to use as store for users config
- Tests run now sequentially
- Testing now success and error
- Skill for AI agents
...
- Replaced winston with a custom logger (decision was made due to the reduction of dependencies)
...
- Fixed spelling, grammar, and phrasing in all documentation (README, cookbook, tools) and CLI descriptions for better
